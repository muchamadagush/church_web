...
class AddStartEndDatetimesToVisitSchedules extends Migration
{
    public function up()
    {
        ...
        $rows = DB::table('visit_schedules')->select('id', 'visit_date')->get();
        foreach ($rows as $row) {
            if ($row->visit_date) {
                $start = Carbon::parse($row->visit_date)->startOfDay(); // 00:00:00
                $end = Carbon::parse($row->visit_date)->endOfDay();     // 23:59:59
                DB::table('visit_schedules')->where('id', $row->id)->update([
        ...
        // Hapus kolom visit_date jika ada
        if (Schema::hasColumn('visit_schedules', 'visit_date')) {
            Schema::table('visit_schedules', function (Blueprint $table) {
    ...
    {
        // Tambah kembali visit_date (nullable)
        if (!Schema::hasColumn('visit_schedules', 'visit_date')) {
            Schema::table('visit_schedules', function (Blueprint $table) {
        ...
        $rows = DB::table('visit_schedules')->select('id', 'start_datetime')->get();
        foreach ($rows as $row) {
        ...
        Schema::table('visit_schedules', function (Blueprint $table) {
            if (Schema::hasColumn('visit_schedules', 'start_datetime')) {
                $table->dropColumn('start_datetime');
            }
            if (Schema::hasColumn('visit_schedules', 'end_datetime')) {
